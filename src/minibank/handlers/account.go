...
func getSessionWriter() func(uuid.UUID, string) {
	...
	if models.CassandraEnabled {
		return func(session uuid.UUID, username string) {
			// insert into minibank.sessions(session, username, expiration) values ('foobar', 'foobar',1232343234234234);
			log.Printf("Inserting for username %s\n", username);
			if err := models.CassandraSession.Query(`INSERT INTO minibank.sessions (session, username, expiration) VALUES (?, ?, ?)`,
				session, username, uint64(time.Now().UnixNano()/1000000) + sessionDuration).Exec(); err != nil {
					log.Fatal(err)
				}
		}
	} else {
